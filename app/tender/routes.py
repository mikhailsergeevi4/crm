...
from app.models import Tender, Contract
...
def showTenders():
    ...
    if form.validate_on_submit():
        newTender = Tender(number=form.number.data, end_date=form.end_date.data, game_date=form.game_date.data, ground=form.ground.data, company=form.company.data, notes=form.notes.data, customer=form.customer.data, author=current_user)
...
def editTender(tender_id):
    ...
    if form.validate_on_submit():
        ...
        tender.customer = form.customer.data
        db.session.commit()
...
@bp.route('/tenders/<int:tender_id>/win/', methods=['GET', 'POST'])
@login_required
def winTender(tender_id):
    winTender = Tender.query.filter_by(id=tender_id).one()
    newContract = Contract(number=winTender.number, company=winTender.company, ground=winTender.ground, author=current_user, customer=winTender.customer)
    db.session.delete(winTender)
    db.session.commit()
    flash(Markup("Тендер перемещен в раздел <a href='/contracts'>Контракты</a>"))
    return redirect(url_for('tender.showTenders'))
